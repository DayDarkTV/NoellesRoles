...
public class Noellesroles implements ModInitializer {
    ...
    public static TMMRoles.Role JESTER = trueRegisterRole(new TMMRoles.Role(JESTER_ID,new Color(255,86,243).getRGB() ,true));
    public static TMMRoles.Role MORPHLING =trueRegisterRole(new TMMRoles.Role(MORPHLING_ID, new Color(170, 2, 61).getRGB(),false));
    public static TMMRoles.Role HOST =trueRegisterRole(new TMMRoles.Role(HOST_ID, new Color(255, 205, 84).getRGB(),true));
    public static TMMRoles.Role AWESOME_BINGLUS = trueRegisterRole(new TMMRoles.Role(AWESOME_BINGLUS_ID, new Color(155, 255, 168).getRGB(),true));

    public static TMMRoles.Role BARTENDER =trueRegisterRole(new TMMRoles.Role(BARTENDER_ID, new Color(217,241,240).getRGB(),true));
    public static TMMRoles.Role NOISEMAKER =trueRegisterRole(new TMMRoles.Role(NOISEMAKER_ID, new Color(200, 255, 0).getRGB(),true));
    //public static TMMRoles.Role SWAPPER = trueRegisterRole(new TMMRoles.Role(SWAPPER_ID, new Color(89, 205, 255).getRGB(),true));
    public static TMMRoles.Role PHANTOM =trueRegisterRole(new TMMRoles.Role(PHANTOM_ID, new Color(80, 5, 5, 192).getRGB(),false));
    ...
    public void onInitialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(Noellesroles.MORPH_PACKET, (payload, context) -> {
            ...
            /*if (gameWorldComponent.isRole(context.player(), SWAPPER)) {
                Vec3d swapperPos = context.player().getPos();
                if (payload.player() != null) {
                    if (context.player().getWorld().getPlayerByUuid(payload.player()) != null) {
                        ...
                        context.player().teleport(swappedPos.x, swappedPos.y, swappedPos.z, true);
                        context.player().getWorld().getPlayerByUuid(payload.player()).teleport(swapperPos.x, swapperPos.y, swapperPos.z, true);
                    }
                }
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            dispatcher.register(CommandManager.literal("forceRole").requires(serverCommandSource -> serverCommandSource.hasPermissionLevel(2)).then(CommandManager.argument("player", EntityArgumentType.player()).then(CommandManager.argument("role", StringArgumentType.string()).executes(commandContext -> {
    ...
    public static TMMRoles.Role trueRegisterRole(TMMRoles.Role role) {
        ...
        try {
            ...
            roleAnnouncementIsEvil.put(announcementText, !role.isInnocent());;
            return role;
        } catch (Exception e) {
