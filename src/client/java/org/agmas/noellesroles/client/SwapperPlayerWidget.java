...
public class SwapperPlayerWidget extends ButtonWidget{
    ...
    public SwapperPlayerWidget(LimitedInventoryScreen screen, int x, int y, @NotNull AbstractClientPlayerEntity disguiseTarget, int index) {
        super(x, y, 16, 16, disguiseTarget.getName(), (a) -> {if ((AbilityPlayerComponent.KEY.get(MinecraftClient.getInstance().player)).cooldown == 0) {ClientPlayNetworking.send(new MorphC2SPacket(disguiseTarget.getUuid()));}}, DEFAULT_NARRATION_SUPPLIER);
        this.screen = screen;
